    ...
    foreach($itemList as $item){
        ...
        if(in_array($_SESSION['user']['type'], $roles) || in_array('all', $roles)){
            ...
            if(!empty($itemOptions)){
                ...
                $datato = htmlentities($item->getAttribute('datato'), ENT_QUOTES, 'UTF-8');

                if(isset($datato)){
                    $datato = htmlentities($item->getAttribute('datato'), ENT_QUOTES, 'UTF-8');
                    $first_option = "";
                    foreach ($optionList as $option){ 
                        $first_option = htmlentities($option->getAttribute('value'), ENT_QUOTES, 'UTF-8');
                        break;
                    }
                    $data_table[$datato] = $first_option;
                }


                if(isset($data_table[$name])){                	
                	$table = $db->prepare('SELECT item_type FROM pm_menu WHERE id = ? AND lang = 2 LIMIT 1');
                	//var_dump($table);
                	$table->execute(array($id));
                	$table = $table->fetch();
                	$optionTable = 'pm_' . $table['item_type'];
                	$fieldLabel = "title";
                	$order = "title";        
                }
