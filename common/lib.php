...
function db_countRows($db,$table){
    $result = $db->query('SELECT COUNT(*) as num_rows FROM '.$table.' LIMIT 1');
    return $result->fetch();
}
 ...
 *
 */
function db_prepareUpdate($db, $table, $data, $where=false)
{
    $result = $db->query('SELECT * FROM '.$table.' LIMIT 1');
    $list_cols = db_list_columns($db, $table);
    $count_cols = 0;
...
function add_defaultAppearance($db){
    $appearance_rows = db_countRows($db,'pm_appearance');
    
    $num_rows = $appearance_rows['num_rows'];
    if($num_rows == 0) {
        $data = array();
        $data['header_type'] = 2;
        $data['header_sticky'] = 1;  
        $data['menu_position'] = 3; 
        $data['logo_position'] = 1;
        $data['arrow_position'] = 1;
        $data['booking_search_position'] = 3;
        $data['footer_columns'] = 3;
        $data['user_id'] = $_SESSION['user']['id'];

        $result_appearance = db_prepareInsert($db, 'pm_appearance', $data);
        $result_appearance->execute();
    }
}
...
function db_selectWhere($db,$table,$where='',$order=''){
    $query = "SELECT * FROM pm_" . $table;
    if($where != ''){
        $query .= $where;
    }
    if($order != ''){
