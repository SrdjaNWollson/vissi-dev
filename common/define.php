...
if(is_file(SYSBASE.'common/config.php')){
    ...
    if(ADMIN && is_file($admin_lang_file)){
        ...
        if(is_null($texts)) $texts = @parse_ini_string(file_get_contents($admin_lang_file));
        $result_text = $db->query('SELECT * FROM pm_text WHERE lang = '.$default_lang.' AND display_on = "back" GROUP BY id');
        foreach($result_text as $row) $texts[$row['name']] = $row['value'];
    }
}
...
if(!ADMIN && (MAINTENANCE_MODE == 0  || (isset($_SESSION['user']) && ($_SESSION['user']['type'] == 'administrator' || $_SESSION['user']['type'] == 'manager')))){
    ...
    $texts = array();
    $result_text = $db->query('SELECT * FROM pm_text WHERE lang = '.$id_lang.' AND display_on = "front" GROUP BY id');
